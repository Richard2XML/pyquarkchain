...
class Shard:
    ...
    def __get_block_commit_status(self, block, block_hash=None):
    ...
    async def add_block(self, block):
        ...
        block_hash = block.header.get_hash()
        commit_status, future = self.__get_block_commit_status(block, block_hash)
    ...
    async def add_block_list_for_sync(self, block_list):
        ...
        for block in block_list:
            ...
            self.add_block_futures[block_hash] = self.loop.create_future()
            block_header_list.append(block.header)
            coinbase_amount_map_list.append(block.header.coinbase_amount_map)

        await self.slave.batch_broadcast_xshard_tx_list(
            block_hash_to_x_shard_list, block_list[0].header.branch
        )
        check(len(coinbase_amount_map_list) == len(block_header_list))
        ...
        # Commit all blocks
        for block_header in block_header_list:
            self.state.commit_by_hash(block_header.get_hash())
            Logger.debug("committed mblock {}".format(block_header.get_hash().hex()))

        # Notify all rest add block operations
